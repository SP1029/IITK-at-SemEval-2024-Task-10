    ...
    if np.isnan(float(df_train["Dialogue_Id"][i])):
        ...
        for j in range(prev_idx, curr_idx):
            ...
            if speaker2idx[sp] in speaker_dialogues[int(this_d_id)].keys():
                speaker_dialogues[int(this_d_id)][speaker2idx[sp]].append(
                    utt2idx[utt])
            else:
                speaker_dialogues[int(this_d_id)][speaker2idx[sp]] = [
                    utt2idx[utt]]
        ...
        for sp in speaker_dialogues[int(this_d_id)].keys():
            if len(speaker_dialogues[int(this_d_id)][sp]) < seq2_len:
                for k in range(len(speaker_dialogues[int(this_d_id)][sp]), seq2_len):
                    speaker_dialogues[int(this_d_id)][sp].append(
                        utt2idx["<pad>"])
            elif len(speaker_dialogues[int(this_d_id)][sp]) > seq2_len:
                speaker_dialogues[int(this_d_id)][sp] = speaker_dialogues[int(
                    this_d_id)][sp][:seq2_len]
    ...
    if np.isnan(float(df_test["Dialogue_Id"][i])):
        ...
        for j in range(prev_idx, curr_idx):
            ...
            if speaker2idx[sp] in speaker_indices_test[int(this_d_id)].keys():
                speaker_indices_test[int(
                    this_d_id)][speaker2idx[sp]].append(chat_id)
            else:
                speaker_indices_test[int(this_d_id)][speaker2idx[sp]] = [
                    chat_id]

            if chat_id < seq_len:
                global_speaker_info_test[int(
                    this_d_id)][chat_id] = speaker2idx[sp]

            if speaker2idx[sp] in speaker_dialogues_test[int(this_d_id)].keys():
                speaker_dialogues_test[int(
                    this_d_id)][speaker2idx[sp]].append(utt2idx[utt])
            else:
                speaker_dialogues_test[int(this_d_id)][speaker2idx[sp]] = [
                    utt2idx[utt]]
        ...
        for sp in speaker_dialogues_test[int(this_d_id)].keys():
            if len(speaker_dialogues_test[int(this_d_id)][sp]) < seq_len:
                for k in range(len(speaker_dialogues_test[int(this_d_id)][sp]), seq_len):
                    speaker_dialogues_test[int(this_d_id)][sp].append(
                        utt2idx["<pad>"])
            elif len(speaker_dialogues_test[int(this_d_id)][sp]) > seq_len:
                speaker_dialogues_test[int(this_d_id)][sp] = speaker_dialogues_test[int(
                    this_d_id)][sp][:seq_len]
